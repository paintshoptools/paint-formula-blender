...
export default function InputTable({ formulaName, config, rows, updateRows }) {
  ...
  function getAvailablePigments(currentPigment) {
    ...
    return config.pigments.filter(
      (p) => !selected.includes(p.name) || p.name === currentPigment?.name,
    ); // exclude them
  }
        ...
        updateRows((prev) =>
          prev.map((item) =>
            item.id == id
              ? {
                  ...item,
                  values: item.values.map((oldValue, j) =>
                    j == col ? newValue : oldValue,
                  ),
                }
              : item,
          ),
      ...
      if (v.id == id) {
        if (i > 0) {
          if (i == 1 || !config.shots) {
            ...
            color =
              v.values[i] >= config.fractions[i] / config.fractions[1]
                ? "orange"
                : "";
          }
        }
