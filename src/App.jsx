...
export default function App() {
  ...
  const [inputTableA, setInputTableA] = useState(createDefaultRows(inputConfig));
  const [inputTableB, setInputTableB] = useState(createDefaultRows(inputConfig));
  ...
  */
  ...
  function addRows(factor, row1, row2) {
    if (inputConfig.decimals) {
      ...
      const r1 = row1.values[0] * inputConfig.fractions[1] * p + parseFloat(row1.values[1]) * p;
      const r2 = row2.values[0] * inputConfig.fractions[1] * p + parseFloat(row2.values[1]) * p;
      ...
      return {
        ...
        values: [
            numOunces * 1.0, numShots
          ]
      }       
    } else {
      const result = getRowInTermsOfLCD(row1, inputConfig) + (factor * getRowInTermsOfLCD(row2, inputConfig));
      return {
        id: crypto.randomUUID().slice(0, 8),
        pigment: row1.pigment,
    ...
    >
      ...
      <div>
        <div style={{display:'flex', gap:'0.5rem', padding:'2rem'}}>
          ...
          <Button label = "Subtract" icon='pi pi-minus-circle' onClick = {() => add(-1)}/>
          
          <Button label = "Copy result to Formula A" icon='pi pi-copy' onClick={() => copyTo('A')}/>
          <Button label = "Copy result to Formula B" icon='pi pi-copy' onClick={() => copyTo('B')}/>
          <Button label = "Swap formula A and B" icon='pi pi-arrows-v'onClick={() => swap()}/>
        </div>
      </div>
